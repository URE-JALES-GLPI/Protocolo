        ...
        try{
            if ($id>0) {
                if ($senha!=='') {
                    ...
                    $pdo->prepare("UPDATE usuarios SET nome=?, username=?, email=?, perfil=?, senha_hash=? WHERE id=?")->execute([$nome,$username,$email?:null,$perfil,$hash,$id]);
                } else {
                    $pdo->prepare("UPDATE usuarios SET nome=?, username=?, email=?, perfil=? WHERE id=?")->execute([$nome,$username,$email?:null,$perfil,$id]);
                }
                flash('success','Usuário atualizado.');
            } else {
                if ($senha==='') { flash('error','Senha obrigatória para novo usuário.'); header('Location: usuarios.php'); exit; }
                $hash=password_hash($senha,PASSWORD_DEFAULT);
                $pdo->prepare("INSERT INTO usuarios (nome,username,email,senha_hash,perfil) VALUES (?,?,?,?,?)")->execute([$nome,$username,$email?:null,$hash,$perfil]);
...
$lista=$pdo->query("SELECT * FROM usuarios ORDER BY ativo DESC, nome ASC")->fetchAll();
        ...
        <form method="post">
          ...
          <div class="mb-2"><label class="form-label">Perfil</label>
            <select name="perfil" class="form-select">
              ...
              <option value="admin" <?= ($edit['perfil']??'')==='admin'?'selected':'' ?>>Admin</option>
            </select>
          </div>
        ...
        <table class="table table-hover mb-0">
          ...
          <?php foreach($lista as $u): ?>
            <tr class="<?= $u['ativo']?'':'table-secondary' ?>">
              ...
              <td><span class="badge <?= $u['perfil']==='admin'?'bg-dark':'bg-primary' ?>"><?= h($u['perfil']) ?></span></td>
