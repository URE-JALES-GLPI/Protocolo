...
<body>
<div class="text-center no-print py-3 bg-white shadow-sm">
  <button onclick="window.print()" class="btn btn-primary">🖨️ <?= __('Imprimir / Salvar PDF', 'protocolo') ?></button>
  <a href="<?= Pasta::getFormURLWithID($id) ?>" class="btn btn-secondary"><?= __('Voltar') ?></a>
  <div class="small text-muted mt-2"><?= __('Dica: na janela de impressão, escolha "Salvar como PDF" para gerar o termo sem assinatura. Depois imprima, colete assinaturas e digitalize para upload no sistema.', 'protocolo') ?></div>
</div>
